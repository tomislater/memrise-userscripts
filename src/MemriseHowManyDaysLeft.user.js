...
$( document ).ready(function() {
    $(document).arrive("div.course-progress", function() {
        var newElem = $(this);
		var div_right = $(newElem.children()[0]);
		var span = div_right.children();
		var text = span.text();

		var numbers = text.replace(/[^0-9\/]/g, '').split("/");

		var numerator = parseInt(numbers[0], 10);
		var denominator = parseInt(numbers[1], 10);

		var diff = denominator - numerator;
		var how_many_days = diff / each_day;

		if (how_many_days > 0) {
			span.after("<span> - " + how_many_days + " days left</button>");
		}
    });
});
